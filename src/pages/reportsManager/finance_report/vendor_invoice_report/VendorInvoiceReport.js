...
import { vendorInvoiceReport } from "../../../../redux/_reducers/_reports_reducers";
...
function VendorInvoiceReport() {
  ...
  const {
    vendorInvoiceReportList,
    vendorInvoiceReport_loading,
    vendorInvoiceReportCount,
  ...
  const fetchClientServiceTaskReport = () => {
    dispatch(vendorInvoiceReport(requestPayLoadReturn(true)));
  };
  ...
  const generateExcel = async () => {
    ...
    worksheet.columns = [
      ...
      { header: "Bill Date", key: "billDate", width: 20 }, // element.purchaseDate
      { header: "Vendor Name", key: "vendorName", width: 25 },
      { header: "Description", key: "description", width: 30 },
      { header: "Total Amount", key: "grandTotal", width: 20 },
      { header: "Due", key: "dueAmount", width: 15 },       // grandTotal if status === 'Pending'
    ...
    const response = await reportsServices?.vendorInvoiceReport(
    ...
    const body = response?.data?.docs?.map((data, index) => {
      ...
      return {
        ...
        dueAmount: data?.status === 'Pending' ? data?.grandTotal : 0,
        paidAmount: data?.status !== 'Pending' ? data?.grandTotal : 0,
      };
    });
    body?.forEach((item) => {
  ...
  const generatePDF = async () => {
    ...
    const body = response?.data?.docs?.map((data, index) => {
      return [
        ...
        data.description || '-',
        data.grandTotal || '-',
        data.status === 'Pending' ? data.grandTotal : 0,
        data.status !== 'Pending' ? data.grandTotal : 0,
      ];
    });
    autoTable(doc, {
  ...
  return (
    <GlobalLayout onChange={onChange}>
      <div className="bg-grey-100 w-full p-1">
        ...
        <div className="bg-[#ffffff]  w-full overflow-x-auto mt-1 rounded-xl">
          <table className="w-full max-w-full rounded-xl overflow-x-auto">
            ...
            {vendorInvoiceReport_loading ? (
              ...
              <tbody>
                {vendorInvoiceReportList &&
                  vendorInvoiceReportList?.length > 0 ? (
                  vendorInvoiceReportList?.map((element, index) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="border-none p-2">
                        <div className="max-w-[200px] overflow-hidden text-ellipsis">
                          <Tooltip title={element?.vendorName || '-'}>
                            <span className="whitespace-nowrap">{element?.vendorName || '-'}</span>
                          </Tooltip>
                        </div>
                      </td>
                      ...
                      <td className="border-none p-2">
                        <div className="max-w-[250px] overflow-hidden text-ellipsis">
                          <Tooltip title={element?.description || '-'}>
                            <span className="whitespace-nowrap">{element?.description || '-'}</span>
                          </Tooltip>
                        </div>
                      </td>
                      ...
                      <td className="border-none p-2 text-right">
                        <div className="overflow-hidden ">
                          <Tooltip title={element?.status === 'Pending' ? convertIntoAmount(element?.grandTotal) : '0.00'}>
                            <span className="whitespace-nowrap">{element?.status === 'Pending' ? convertIntoAmount(element?.grandTotal) : '0.00'}</span>
                          </Tooltip>
                        </div>
                      </td>
                          ...
                          >
                            ...
                            <td className="border-none p-2">
                              <div className="max-w-[150px] overflow-hidden text-ellipsis">
                                <Tooltip title={dayjs(element?.createdAt).format('DD-MM-YYYY hh:mm a')}>
                                  <span className="whitespace-nowrap">{dayjs(element?.createdAt).format('DD-MM-YYYY hh:mm a')}</span>
                                </Tooltip>
                              </div>
                            </td>
                            ...
                            <td className="border-none p-2">
                              <div className="max-w-[150px] overflow-hidden text-ellipsis">
                                <Tooltip title={dayjs(element?.date).format('DD-MM-YYYY hh:mm a')}>
                                  <span className="whitespace-nowrap">{dayjs(element?.date).format('DD-MM-YYYY hh:mm a')}</span>
                                </Tooltip>
                              </div>
                            </td>
                            ...
                            <td className="border-none p-2">
                              <div className="max-w-[200px] overflow-hidden text-ellipsis">
                                <Tooltip title={vendorListData?.fullName || "-"}>
                                  <span className="whitespace-nowrap">{vendorListData?.fullName || "-"}</span>
                                </Tooltip>
                              </div>
                            </td>
                            ...
                            <td className="border-none p-2">
                              <div className="max-w-[250px] overflow-hidden text-ellipsis">
                                <Tooltip title={element?.naration || "-"}>
                                  <span className="whitespace-nowrap">{element?.naration || "-"}</span>
                                </Tooltip>
                              </div>
                            </td>
                            ...
                            <td className="border-none p-2 text-right">
                              <div className="max-w-[120px] overflow-hidden text-ellipsis">
                                <Tooltip title={convertIntoAmount(element?.amount) || "-"}>
                                  <span className="whitespace-nowrap">{convertIntoAmount(element?.amount) || "-"}</span>
                                </Tooltip>
                              </div>
                            </td>
                            ...
                            <td className="border-none p-2">
                              <div className="max-w-[120px] overflow-hidden text-ellipsis">
                                <Tooltip title={element?.type}>
                                  <span className="whitespace-nowrap">{element?.type}</span>
                                </Tooltip>
                              </div>
                            </td>
        ...
        <CustomPagination
          totalCount={vendorInvoiceReportCount}
          pageSize={pageSize}
          currentPage={currentPage}
