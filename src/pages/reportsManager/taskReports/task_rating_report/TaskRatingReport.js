...
import { Collapse, Flex, Modal, Rate, Select, Tooltip } from "antd";
...
function TaskRatingReport() {
  ...
  const {
    taskRatingReportList,
    taskRatingReportFunc_loading,
    totalRatingTaskTaskCount,
  ...
  const fetchOverAllTaskReport = () => {
    dispatch(taskRatingReportFunc(requestPayLoadReturn(true)));
  };
  ...
  const generateExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("TaskRatingReport");
    ...
    worksheet.columns = [
      ...
      { header: "Client Comment", key: "feedback", width: 30 },
    ...
    const response = await reportsServices?.taskRatingReportFunc(
      requestPayLoadReturn(false)
    );


    if (!taskRatingReportList && !response) return;
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "TaskRatingReport.xlsx";
      link.click();
    });
  };
  ...
  return (
    <GlobalLayout onChange={onChange}>
      <div className="bg-grey-100 w-full p-1">
        ...
        <div className="bg-[#ffffff]  w-full overflow-x-auto mt-1 rounded-xl">
          <table className="w-full max-w-full rounded-xl overflow-x-auto">
            ...
            {taskRatingReportFunc_loading ? (
              ...
              <tbody>
                {taskRatingReportList && taskRatingReportList?.length > 0 ? (
                  taskRatingReportList?.map((element, index) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap text-center border-none p-2">
                        <Flex gap="middle" vertical>
                          <Rate disabled defaultValue={element?.ratingData?.rating || 0} />
                        </Flex>

                      </td>
                      <td className="whitespace-nowrap text-center border-none p-2">
                        {element?.ratingData?.feedback || "-"}
                      </td>
        ...
        <CustomPagination
          totalCount={totalRatingTaskTaskCount}
          pageSize={pageSize}
          currentPage={currentPage}
...
export default TaskRatingReport;
