...
import { lastUpdateTaskReportFunc } from "../../../../redux/_reducers/_reports_reducers";
import { BsFileEarmarkPdfFill } from "react-icons/bs";

function LastTaskUpdationReport() {
  const {
    control,
    formState: { errors },
    setValue,
  ...
  const {
    lastUpdateTaskReportList,
    lastUpdateTaskReportFunc_loading,
    totallastUpdateTaskTaskCount,
  ...
  const fetchOverAllTaskReport = () => {
    dispatch(lastUpdateTaskReportFunc(requestPayLoadReturn(true)));
  };
  ...
  const generateExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Last Update Task");
    ...
    worksheet.columns = [
      ...
      { header: "Comment", key: "commentData", width: 25 },
      { header: "Attachment", key: "attachment", width: 15 },
    ];
    ...
    const response = await reportsServices?.lastUpdateTaskReportFunc(
      requestPayLoadReturn(false)
    );
    if (!lastUpdateTaskReportList && !response) return;
    const apiData = response?.data?.docs?.map((data, index) => {
      return {
        sno: index + 1,
        date: data?.commentData?.createdAt ? dayjs(data?.commentData?.createdAt).format("DD-MM-YYYY") : "-",
        ...
        commentData: data?.commentData?.message || "-",
        attachment: data?.commentData?.attachment ? "Yes" : "No"
      };
    });
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "Last_Update_Task.xlsx";
      link.click();
    });
  };
  const generatePDF = async () => {
    ...
    const body = response?.data?.docs?.map((data, index) => {
      return [
        index + 1,
        data?.commentData?.createdAt ? dayjs(data?.commentData?.createdAt).format("DD-MM-YYYY") : "N/A",
        ...
        data?.commentData?.message || "N/A",
        data?.commentData?.attachment ? "Yes" : "No"
      ];
    });
    autoTable(doc, {
      startY: 40,
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    ...
                    {/* <Select.Option value="">Select client</Select.Option>
                  {
                    sortByPropertyAlphabetically(clientList)?.map(
                      (element) => (
                        <Select.Option key={element?._id} value={element?._id}>
                          {element?.fullName}
                        </Select.Option>
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              <Select
                                ...
                                placeholder="Please select Task Status"
                                options={CLIENT_TASK_STATUS_ARR?.map((array) => {
                                  return { value: array, label: array?.replace(/_/g, ' ')
                          .toLowerCase()
                          .replace(/\b\w/g, char => char.toUpperCase()) };
                                })}
                                value={field.value || []}
  ...
  return (
    <GlobalLayout onChange={onChange}>
      <div className="bg-grey-100 w-full p-1">
        ...
        <div className="bg-[#ffffff]  w-full overflow-x-auto mt-1 rounded-xl">
          <table className="w-full max-w-full rounded-xl overflow-x-auto">
            ...
            {lastUpdateTaskReportFunc_loading ? (
              ...
              <tbody>
                {lastUpdateTaskReportList && lastUpdateTaskReportList?.length > 0 ? (
                  lastUpdateTaskReportList?.map((element, index) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap text-center border-none p-2">
                        {element?.commentData?.createdAt ? dayjs(element?.commentData?.createdAt).format('DD-MM-YYYY') : '-'}
                      </td>
                      <td className="whitespace-nowrap text-center border-none p-2">
                      ...
                      <td className="">
                        <Tooltip placement="topLeft"  title="Attachment">
                          ...
                          <button
                            onClick={() => {
                              if (element?.commentData?.attachment) {
                                const url = `${process.env.REACT_APP_BACKEND_DOMAIN_NAME}/public${element?.commentData?.attachment}`;
        ...
        <CustomPagination
          totalCount={totallastUpdateTaskTaskCount}
          pageSize={pageSize}
          currentPage={currentPage}
...
export default LastTaskUpdationReport;
