...
import { overAllTaskReportFunc, workingHourTaskReportFunc } from "../../../../redux/_reducers/_reports_reducers";
...
function WorkingHourTaskReport() {
  const {
    control,
    formState: { errors },
    setValue,
  ...
  const {
    workingHourTaskReportList,
    workingHourTaskReportFunc_loading,
    totalWorkingHourTaskTaskCount,
  ...
  const fetchOverAllTaskReport = () => {
    dispatch(workingHourTaskReportFunc(requestPayLoadReturn(true)));
  };
  ...
  const generateExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("workingHoursReport");
    ...
    const response = await reportsServices?.workingHourTaskReportFunc(
      requestPayLoadReturn(false)
    );


    if (!workingHourTaskReportList && !response) return;
    const apiData = response?.data?.docs?.map((data, index) => {
      return {
        ...
        startDate: dayjs(data?.createdAt).format('DD-MM-YYYY hh:mm a'),
        completedDate: data?.workingHoursData ? dayjs(data?.workingHoursData?.createdAt).format('DD-MM-YYYY hh:mm a') : '',
        workingHours: data?.workingHoursData ? data?.workingHoursData?.workedHRS : '-',
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "workingHoursReport.xlsx";
      link.click();
    });
  };
  const generatePDF = async () => {
    ...
    const body = response?.data?.docs?.map((data, index) => {
      return [
        ...
        data?.workingHoursData ? dayjs(data?.workingHoursData?.createdAt).format('DD-MM-YYYY hh:mm a') : '-',
        data?.workingHoursData ? data?.workingHoursData?.workedHRS : '-',
      ];
    });
    autoTable(doc, {
      startY: 40,
    ...
    doc.save(`workingHoursReport.pdf`);
  };
  ...
  return (
    <GlobalLayout onChange={onChange}>
      <div className="bg-grey-100 w-full p-1">
        ...
        <div className="bg-[#ffffff]  w-full overflow-x-auto mt-1 rounded-xl">
          <table className="w-full max-w-full rounded-xl overflow-x-auto">
            ...
            {workingHourTaskReportFunc_loading ? (
              ...
              <tbody>
                {workingHourTaskReportList && workingHourTaskReportList?.length > 0 ? (
                  workingHourTaskReportList?.map((element, index) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap text-center border-none p-2">
                        <div className="flex gap-2">
                          <div> {element?.employeData[0]?.fullName}</div>  {element?.employeData?.length > 1 ? <div className="text-header flex justify-center items-center gap-1 rounded-full  text-semibold text-sm"> + <div onClick={() => { handleEmployeeModal(element?.employeData) }} className="border justify-center items-center flex h-7 w-7 bg-header  text-white text-[12px] rounded-full ">{element?.employeData?.length - 1} </div></div> : ''}
                      ...
                      <td className="whitespace-nowrap text-center border-none p-2">
                        {element?.workingHoursData?.createdAt ? `${element?.workingHoursData?.createdAt} hrs` : '-'}
                      </td>

                      <td className="whitespace-nowrap text-center border-none p-2">
                        {element?.workingHoursData?.workedHRS ? `${element?.workingHoursData?.workedHRS} hrs` : '-'}
                      </td>
                    </tr>
                  ))
                ) : (
        ...
        <CustomPagination
          totalCount={totalWorkingHourTaskTaskCount}
          pageSize={pageSize}
          currentPage={currentPage}
...
export default WorkingHourTaskReport;