...
import { overDueTaskReportFunc } from "../../../../redux/_reducers/_reports_reducers";
...
function OverDueTaskReport() {
  ...
  const fetchOverAllTaskReport = () => {
    dispatch(overDueTaskReportFunc(requestPayLoadReturn(true)));
  };
  ...
  const generateExcel = async () => {
    ...
    const response = await reportsServices?.overDueTaskReportFunc(
      requestPayLoadReturn(false)
    );


    if (!overDueReportList && !response) return;
  ...
  return (
    <GlobalLayout onChange={onChange}>
      <div className="bg-grey-100 w-full p-1">
        ...
        <div className="bg-[#ffffff]  w-full overflow-x-auto mt-1 rounded-xl">
          <table className="w-full max-w-full rounded-xl overflow-x-auto">
            ...
            {overDueTaskReportFunc_loading ? (
              ...
              <tbody>
                {overDueReportList && overDueReportList?.length > 0 ? (
                  overDueReportList?.map((element, index) => (
                    <tr
                    ...
                    >
                        ...
                        {element?.isOverDue ? 
                          (element?.dueDate ? 
                            <span className="px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
                              {dayjs().diff(dayjs(element.dueDate), 'days')} days overdue
                            </span> : 
                            <span className="px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
                              Overdue
                            </span>
                      ...
                      <td className="whitespace-nowrap text-center border-none p-2">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          element?.status?.toLowerCase() === 'in_progress' || element?.status?.toLowerCase() === 'work_in_progress' ? 'bg-blue-100 text-blue-800' :
                          element?.status?.toLowerCase() === 'in_review' ? 'bg-purple-100 text-purple-800' :
                          element?.status?.toLowerCase() === 'assigned' ? 'bg-indigo-100 text-indigo-800' :
                          element?.status?.toLowerCase() === 'accepted' ? 'bg-emerald-100 text-emerald-800' :
                          element?.status?.toLowerCase() === 'pending_at_client' ? 'bg-amber-100 text-amber-800' :
                          element?.status?.toLowerCase() === 'pending_at_department' ? 'bg-violet-100 text-violet-800' :
                          element?.status?.toLowerCase() === 'pending_at_colleague' ? 'bg-teal-100 text-teal-800' :
                          element?.status?.toLowerCase() === 'pending_at_manager' ? 'bg-orange-100 text-orange-800' :
                          element?.status?.toLowerCase() === 'pending_for_approval' ? 'bg-pink-100 text-pink-800' :
                          element?.status?.toLowerCase() === 'pending_for_fees' ? 'bg-gray-100 text-gray-800' :
                          element?.status?.toLowerCase() === 'task_stop' ? 'bg-red-100 text-red-800' :
                          element?.status?.toLowerCase() === 'reassign_to_other' ? 'bg-cyan-100 text-cyan-800' :
        ...
        <CustomPagination
          totalCount={totaloverDueTaskCount}
          pageSize={pageSize}
          currentPage={currentPage}
...
export default OverDueTaskReport;
