...
import { employeeAppraisalReportFunc } from "../../../../redux/_reducers/_reports_reducers";
...
import ClientListModal from "../../../client/clientGroup/ClientListModal";
import { FaEye } from "react-icons/fa";
import AppraisalListModal from "./AppraisalListModal";
import ListLoader from "../../../../global_layouts/ListLoader";

function EmployeeAppraisalReport() {
  const { control, formState: { errors }, setValue } = useForm();
  const dispatch = useDispatch();
  const { employeeAppraisalReportist, employeeAppraisalReportFunc_loading, totalemployeeAppraisalReportCount, } = useSelector((state) => state.reports);
  ...
  const [openAppraisalModal, setOpenAppraisalModal] = useState({
    data: null,
    isOpen: false
  })
  ...
  const requestPayLoadReturn = (pagination = true) => {
    return {
      ...
      reqPayload: {
        ...
        designationIds: designationId ? [designationId] : [],
  ...
  const fetchEmpAppraisalListData = () => {
    dispatch(employeeAppraisalReportFunc(requestPayLoadReturn(true)));
  };
  ...
  const generateExcel = async () => {
    ...
    worksheet.columns = [
      ...
      { header: "Employee Name", key: "employeName", width: 30 },
      { header: "Designation", key: "designationName", width: 20 },
      { header: "Department", key: "departmentName", width: 20 },
      { header: "Current Salary", key: "currentSalary", width: 20 },
      { header: "Previous Salary", key: "previousMonthlySalary", width: 25 },
      { header: "Date of Appraisal", key: "lastIncrementDate", width: 25 },
      { header: "Date of next increment", key: "nextIncrementDate", width: 25 },
    ];
    ...
    const response = await reportsServices?.employeeAppraisalReportFunc(requestPayLoadReturn(false));
    if (!employeeAppraisalReportist && !response) return;
    const apiData = response?.data?.docs?.map((data, index) => {
      return {
        ...
        previousMonthlySalary: data?.latestIncrement?.previousMonthlySalary || "-",
        lastIncrementDate: data?.lastIncrementDate
          ? dayjs(data.lastIncrementDate).format("DD-MM-YYYY hh:mm A")
          : "-",
        nextIncrementDate: data?.nextIncrementDate
          ? dayjs(data.nextIncrementDate).format("DD-MM-YYYY hh:mm A")
          : "-",
      };
    });
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      link.download = "Employee_Appraisal_Report.xlsx";
      link.click();
    });
  };
  const generatePDF = async () => {
    ...
    const body = response?.data?.docs?.map((data, index) => {
      return [
        ...
        data?.departmentName || "-",
        data?.currentSalary || "-",
        data?.latestIncrement?.previousMonthlySalary || "-",
    ...
    doc.save(`Employee_Appraisal_Report.pdf`);
  }
  const items = [
    {
      key: "1",
  ...
  return (
    <GlobalLayout onChange={onChange}>
      <div className="bg-grey-100 w-full p-1">
          ...
          <table className="w-full max-w-full rounded-xl overflow-x-auto">
            <thead>
              <tr className="border-b-[1px] border-[#DDDDDD] capitalize text-[12px] bg-header text-white font-[500] h-[40px]">
                ...
                <th className="border-none p-2 whitespace-nowrap text-center">Designation</th>
                <th className="border-none p-2 whitespace-nowrap text-center">Department</th>
                ...
                <th className="border-none p-2 whitespace-nowrap text-center">Previous Salary </th>
                <th className="border-none p-2 whitespace-nowrap text-center">Date of Appraisal</th>
                <th className="border-none p-2 whitespace-nowrap text-center">Date of next increment </th>

              </tr>
            </thead>
            {employeeAppraisalReportFunc_loading ? (
              ...
              <tbody>
                {employeeAppraisalReportist &&
                  employeeAppraisalReportist?.length > 0 ? (
                  employeeAppraisalReportist?.map((element, index) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap text-center border-none px-2 py-3">
                        {element?.latestIncrement?.previousMonthlySalary || "-"}
                      </td>


                      <td className="whitespace-nowrap text-center border-none px-2 py-3">
                        {dayjs(element?.lastIncrementDate).format('DD-MM-YYYY hh:mm a') || "-"}
                      </td>
                      <td className="whitespace-nowrap text-center border-none px-2 py-3">
                        {dayjs(element?.nextIncrementDate).format('DD-MM-YYYY hh:mm a') || "-"}
                      </td>
        ...
        <CustomPagination
          totalCount={totalemployeeAppraisalReportCount}
          pageSize={pageSize}
          currentPage={currentPage}
      ...
      <AppraisalListModal
        ...
        isOpen={openAppraisalModal?.isOpen}
        employeData={openAppraisalModal?.data}
        onClose={() => {
...
export default EmployeeAppraisalReport;
