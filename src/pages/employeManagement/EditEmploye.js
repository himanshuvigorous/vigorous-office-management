...
const EditEmploye = () => {
  ...
  useEffect(() => {
    if (employeDetailsData?.data) {
      ...
      // generalInfo
      setValue("PDDateOfBirth", dayjs(employeDetailsData?.data?.generalInfo?.dateOfBirth));
      ...
      if (employeDetailsData?.data?.socialLinks?.length > 0) {
        setValue("SMInstagram", employeDetailsData?.data?.socialLinks?.find((element) => element?.name === "Instagram")?.link);
        setValue("SMTwitter", employeDetailsData?.data?.socialLinks?.find((element) => element?.name === "Twitter")?.link);
        setValue("SMFacebook", employeDetailsData?.data?.socialLinks?.find((element) => element?.name === "Facebook")?.link);
        setValue("SMWebsite", employeDetailsData?.data?.socialLinks?.find((element) => element?.name === "Website")?.link);
        setValue("SMLinkedIn", employeDetailsData?.data?.socialLinks?.find((element) => element?.name === "linkedin")?.link);
        ...
        >
          ...
          {step === 3 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 md:gap-8 md:my-1 px-3">
                <div className="w-full">
                  <label className={`${inputLabelClassName}`}>Linkedin Link</label>
                  <input
                    type="url"
                    {...register("SMLinkedIn")}
                    className={` ${inputClassName} ${errors.SMLinkedIn ? "border-[1px] " : "border-gray-300"
                      }`}
