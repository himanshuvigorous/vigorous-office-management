...
export const getLeaveDashboard = createAsyncThunk(
  "user/getLeaveDashboard",
  async (reqData, { rejectWithValue }) => {

    try {
      const response = await leaveRequestServices.getLeaveDashboard(reqData);
      showNotification({
        message: response?.message,
        type: 'success',
      });
...
const employeeDocumentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getLeaveDashboard.pending, (state) => {
        state.loading = true;
        state.leaveDashboardData = null;
      })
      .addCase(getLeaveDashboard.fulfilled, (state, action) => {
        state.loading = false;
        state.leaveDashboardData = action.payload?.data;
      })
      .addCase(getLeaveDashboard.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
        // state.leaverequestDetails = null;
      })
