...
import {
  deductionsDelete,
  getdeductionsList,
} from "./deductionsfeature/_deductionsList_reducers";
...
function DeductionsList() {
  ...
  const { deductionsListData, deductionsListTotal, loading } = useSelector(
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deductionsDelete(reqData)).then((data) => {
          // getDeductionsData();
          if (currentPage > 1 && deductionsListData?.length == 1) {
    ...
    <GlobalLayout onChange={onChange}>
      <div className="bg-grey-100 w-full p-1">
          ...
          <div className="flex justify-end items-center gap-2 ">
            ...
            {canCreate && canRead && (
              <Tooltip placement="topLeft"  title="Create Deduction">
                <button
                  onClick={() => navigate("/admin/deductions/create")}
                  className="bg-header px-2 py-1.5 rounded-md flex justify-center items-center space-x-2 text-white"
                >
                  <FaPlus />
                  <span className="text-[12px]">Create Deductions</span>
                </button>
              </Tooltip>
            )}
          </div>
        ...
        <div className="w-full">
            ...
            {canRead && (
              <table className="w-full max-w-full rounded-xl overflow-x-auto">
                ...
                {loading ? (
                  ...
                  <tbody>
                    {deductionsListData && deductionsListData?.length > 0 ? (
                      deductionsListData?.map((element, index) => (
                        <tr
          ...
          <CustomPagination
            totalCount={deductionsListTotal}
            pageSize={pageSize}
            currentPage={currentPage}
