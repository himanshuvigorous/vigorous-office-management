...
import {
  ...
  getDepartmentList,
  deptSearch,
} from "./departmentFeatures/_department_reducers";
...
function DepartmentList() {
  ...
  const { departmentListData, totalDepartmentCount, loading } = useSelector(
  ...
  const fetchDepartmentList = () => {
    ...
    dispatch(getDepartmentList(reqListData));
  };
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteDepartmentFunc(reqData)).then((data) => {
          // !data.error && dispatch(fetchDepartmentList());
          if (currentPage > 1 && departmentListData?.length == 1) {
            setCurrentPage(Number(currentPage - 1));
          } else {
            !data.error && fetchDepartmentList()
          }
        });
      }
    });
        ...
        <div className="">
          ...
          <div className="sm:flex justify-between items-center md:space-y-0 space-y-2 py-1">
            ...
            <div className="flex justify-end items-center gap-2 ">
              ...
              {(canRead && canCreate) &&
                <Tooltip placement="topLeft"  title="New Department">
                  <button
                    onClick={() => {
                  ...
                  >
                    <FaPlus />
                    <span className="text-[12px]">Add Department</span>
                  </button>
                </Tooltip>}
            </div>
          </div>
