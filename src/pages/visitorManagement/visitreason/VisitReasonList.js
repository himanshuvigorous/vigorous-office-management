...
import { deleteVisitReasonType, getVisitReasonList } from "./visitReasonFeatures/_visitReason_type_reducers";
import { branchSearch } from "../../branch/branchManagement/branchFeatures/_branch_reducers";
import { companySearch } from "../../company/companyManagement/companyFeatures/_company_reducers";
import GlobalLayout from "../../../global_layouts/GlobalLayout/GlobalLayout";
import ListLoader from "../../../global_layouts/ListLoader";

function VisitReasonList() {
  const {
    register,
    handleSubmit,
    control,
  ...
  const { VisitReasonListData, totaVisitReasonTypeCount, loading } = useSelector((state) => state.visitReason);
  ...
  const getVisitReason = () => {
    ...
    dispatch(getVisitReasonList(data));
  };
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteVisitReasonType(reqData)).then((data) => {
          getVisitReason()
        });
      }
    });
      ...
      <>
        <div className="w-full">
          <div className="sm:flex justify-between items-center md:space-y-0 space-y-2 ">
            <div className="grid grid-cols-1 items-center md:grid-cols-4 md:gap-2 gap-1">
            ...
            <div className="flex justify-end items-center gap-2 ">
              ...
              {canCreate &&
                <Tooltip placement="topLeft"  title='Add Visit Reason'>
                  <button
                    onClick={() => {
                      navigate("/admin/visit-reason/create");
                    }}
                    className="bg-[#074173] px-2 py-1.5 rounded-md flex justify-center items-center space-x-2 text-white"
                  >
                    <FaPlus />
                    <span className="text-[12px]">Add Visit Reason</span>
                  </button>
                </Tooltip>
              }
            </div>
          ...
          {canRead && <table className="w-full max-w-full rounded-xl overflow-x-auto ">
            <thead className="">
              <tr className="border-b-[1px] border-[#DDDDDD] bg-[#074173] capitalize text-[12px] text-[#ffff] font-[500] h-[40px]">
                ...
                <th className="p-2 whitespace-nowrap">Visit Reason</th>
            ...
            {loading ? <tr className="bg-white bg-opacity-5 ">
              ...
              <tbody>
                {VisitReasonListData && VisitReasonListData?.length > 0 ? (
                  VisitReasonListData?.map((element, index) => (
                    <tr
                        ...
                        <span className="py-1.5 flex justify-start items-center space-x-2.5">
                          {canUpdate && <Tooltip placement="topLeft"  title='Edit'>
                            <button
                              onClick={() => {
                                navigate(
                                  `/admin/visit-reason/edit/${encrypt(element?._id)}`
                                );
                              }}
        ...
        {totaVisitReasonTypeCount > limit && (
          <CustomPagination
            totalCount={totaVisitReasonTypeCount}
            pageSize={limit}
            currentPage={currentPage}
