...
import { deleteServerManagement, getServerManagementList } from "./serverManagementFeatures/_server-management_reducers";
...
function ServerList() {
    ...
    const { ServerManagementList, loading, totalServerManagementCount } = useSelector(state => state.serverManagement)
    ...
    const filters = [ BranchId, status, searchText].join("-");
    ...
    const fetchServerManagementList = () => {
        const reqData = {
            page: currentPage,
            limit: limit,
            reqPayload: {
        ...
        dispatch(getServerManagementList(reqData))
    }
    ...
    const handleDelete = (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(deleteServerManagement(reqData)).then((data) => {
                   

                    if (currentPage > 1 && ServerManagementList?.docs?.length == 1) {
    ...
    return (
        <GlobalLayout onChange={onChange}>
            ...
            <section>
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                ...
                                >
                                    ...
                                    {['Active', 'Maintenance', 'Decommissioned', 'Provisioning']?.map((item) => (
                ...
                <div className="bg-[#ffffff] w-full overflow-x-auto mt-1 rounded-xl">
                    {canRead && <table className="w-full max-w-full rounded-xl overflow-x-auto ">
                        ...
                        {loading ? <tr className="bg-white bg-opacity-5 ">
                            ...
                            <tbody>
                                {ServerManagementList && ServerManagementList?.docs?.length > 0 ?
                                    ServerManagementList?.docs?.map((element, index) => (
                                        <tr className={`border-b-[1px] border-[#DDDDDD] ${index % 2 === 0 ? 'bg-[#e9ecef]/80' : 'bg-white'} text-[#374151] text-[14px]`}>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{index + 1 + ((currentPage - 1) * limit)}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{element?.credentials?.username ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{element?.serverName ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{element?.operatingSystem?.name ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{element?.operatingSystem?.version ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{element?.hostingProvider?.provider ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{element?.hostingProvider?.instanceType ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{dayjs(element?.serverCreateDate).format('DD-MM-YYYY') ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 !w-[150px] '>{element?.createdBy ?? "-"}</td>
                                            <td className='whitespace-nowrap border-none p-2 '>
                                                <span
                                                    className={` border-[1px] bg-header px-2 py-1.5 rounded-lg text-white text-[12px]`}
                                                >
                                                    {element?.status || "-"}
                                                </span>
                                            </td>
                                            {(canUpdate || canDelete) && <td className='whitespace-nowrap border-none p-2'>
                                                <span className="py-1.5 flex justify-start items-center space-x-2">
                                                    {canUpdate && <Tooltip placement="topLeft"  title='Edit'>
                                                        <button
                                                            onClick={() => {
                                                                navigate(`/admin/server-management/update/${encrypt(element?._id)}`);
                                                            }}
                ...
                <CustomPagination
                    totalCount={totalServerManagementCount}
                    pageSize={limit}
