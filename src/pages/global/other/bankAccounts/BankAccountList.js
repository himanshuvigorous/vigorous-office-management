...
import { deletebankAccount, getbankAccountList } from "./bankAccountFeature/_bank_account_reducers";
...
function BankAccountList() {
  ...
  const { bankAccountListData, totalbankAccountListCount, loading } = useSelector(
  ...
  const getBankAccountListRequest = () => {
    const data = {
    ...
    dispatch(getbankAccountList(data));
  };
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deletebankAccount(reqData)).then((data) => {
          // getBankAccountListRequest()

          if (currentPage > 1 && bankAccountListData?.length == 1) {
  ...
  return (
    <GlobalLayout onChange={onChange}>
      ...
      <>
        <div className="">
          <div className="sm:flex justify-between items-center md:space-y-0 space-y-2 py-1">
            ...
            <div className="flex justify-end items-center gap-2 ">
              ...
              {canCreate &&
                <Tooltip placement="topLeft"  title='Add Bank Account'>
                  <button
                    onClick={() => {
                      navigate("/admin/bank-account/create");
                    }}
                    className="bg-header px-2 py-1.5 rounded-md flex justify-center items-center space-x-2 text-white"
                  >
                    <FaPlus />
                    <span className="text-[12px]">Add Bank Account</span>
                  </button>
                </Tooltip>}
            </div>
          </div>
          ...
          {canRead && <table className="w-full max-w-full rounded-xl overflow-x-auto ">
            <thead className="">
              <tr className="border-b-[1px] border-[#DDDDDD] bg-header capitalize text-[12px] text-[#ffff] font-[500] h-[40px]">
                ...
                <th className="p-2 whitespace-nowrap">Bank holder name</th>
                <th className="p-2 whitespace-nowrap">Account Type</th>
            ...
            {loading ? <tr className="bg-white bg-opacity-5 ">
              ...
              <tbody>
                {bankAccountListData && bankAccountListData?.length > 0 ? (
                  bankAccountListData?.map((element, index) => (
                    <tr
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap p-2">{element?.bankName}</td>
                      ...
                      {(canDelete || canUpdate) && <td className="whitespace-nowrap p-2">
                          ...
                          {canUpdate &&
                            <Tooltip placement="topLeft"  title='Edit'>
                              <button
                                onClick={() => {
                                  navigate(
                                    `/admin/bank-account/edit/${encrypt(element?._id)}`
                                  );
                                }}
        ...
        {bankAccountListData?.length > 0 &&
          <CustomPagination
            totalCount={totalbankAccountListCount}
            pageSize={limit}
            currentPage={currentPage}
