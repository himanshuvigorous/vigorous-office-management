...
import { deleteGstTypeFunc, getGstTypeListFunc } from "./GstTypeFeatures/_gstType_reducers"
...
function GstType() {
  ...
  const { gstTypeList, totalGstTypeCount, loading } = useSelector(state => state.gstType)
  ...
  const fetchGstType = () => {
    ...
    dispatch(getGstTypeListFunc(reqData))
  }
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteGstTypeFunc(reqData)).then((data) => {

          // fetchGstType()


          if (currentPage > 1 && gstTypeList?.length == 1) {
          ...
          <div className="flex justify-end items-center gap-2">
            ...
            {canCreate &&
              <Tooltip placement="topLeft"  title='Add GST Type'>
                <button onClick={() => { navigate("/admin/gst-type/create") }} className='bg-header px-2 py-1.5 rounded-md flex justify-center items-center space-x-2 text-white'>
                  <FaPlus />
                  <span className='text-[12px]'>Add GST Types</span>
                </button>
              </Tooltip>}
          </div>
        </div>
      ...
      <div className="bg-[#ffffff] w-full overflow-x-auto mt-1 rounded-xl">
        {canRead && <table className="w-full max-w-full rounded-xl overflow-x-auto ">
          ...
          {loading ? <tr className="bg-white bg-opacity-5 ">
            ...
            <tbody>
              {gstTypeList && gstTypeList?.length > 0 ?
                gstTypeList?.map((element, index) => (
                  <tr className={`${index % 2 === 0 ? 'bg-[#e9ecef]/80' : 'bg-white'} border-b-[1px] border-[#DDDDDD] text-[#374151] text-[14px]`}>
                    ...
                    <td className='whitespace-nowrap border-none p-2 '>{`${element?.percentage}% ` ?? "-"}</td>
                    ...
                    {(canUpdate || canDelete) && <td className='whitespace-nowrap border-none p-2'>
                      <span className="py-1.5 flex justify-start items-center space-x-2">
                        {canUpdate && <button onClick={() => { navigate(`/admin/gst-type/edit/${encrypt(element?._id)}`) }} className="px-2 py-1.5 text-xs rounded-md bg-transparent border border-muted" type="button">
                          <FaPenToSquare className=' hover:text-[#337ab7] text-[#3c8dbc]' size={16} />
                        </button>}
                        {canDelete && <button onClick={() => handleDelete(element?._id)} className="px-2 py-1.5 rounded-md bg-transparent border border-muted" type="button">
                ...
                : (<tr className="bg-white bg-opacity-5 " >
                  <td colSpan={5} className="px-6 py-2 whitespace-nowrap font-[600] text-center text-sm text-gray-500">Record Not Found</td>
                </tr>)}
            </tbody>}
