...
import { deleteInterviewRoundType, getInterviewRoundList } from "./InterviewRoundFeatures/_interviewRound_type_reducers";
...
function InterviewRoundList() {
  ...
  const { interviewRoundListData, totaInterviewRoundCount, loading } = useSelector((state) => state.interviewRound);
  ...
  const getInterviewRound = () => {
    ...
    dispatch(getInterviewRoundList(data));
  };
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteInterviewRoundType(reqData)).then((data) => {
          // getInterviewRound()
          if (currentPage > 1 && interviewRoundListData?.length == 1) {
  ...
  return (
    <GlobalLayout onChange={onChange}>
      ...
      <>
        <div className="">
          ...
          <div className="sm:flex justify-between items-center md:space-y-0 space-y-2 py-1">
            ...
            <div className="flex justify-end items-center gap-2 ">
              ...
              {canCreate &&
                <Tooltip placement="topLeft"  title='Interview Round'>
                  <button
                    onClick={() => {
                      navigate("/admin/interview-round-list/create");
                    }}
                    className="bg-[#074173] px-2 py-1.5 rounded-md flex justify-center items-center space-x-2 text-white"
                  >
                    <FaPlus />
                    <span className="text-[12px]">Add Interview Round</span>
                  </button>
                </Tooltip>}
            </div>
          </div>
        ...
        <div className="bg-[#ffffff] w-full overflow-x-auto mt-1 rounded-xl">
          {canRead && <table className="w-full max-w-full rounded-xl overflow-x-auto ">
            ...
            {loading ? <tr className="bg-white bg-opacity-5 ">
              ...
              <tbody>
                {interviewRoundListData && interviewRoundListData?.length > 0 ? (
                  interviewRoundListData?.map((element, index) => (
                    <tr
                        ...
                        <span className="py-1.5 flex justify-start items-center space-x-2.5">
                          {canUpdate && <Tooltip placement="topLeft"  title='Edit'>
                            <button
                              onClick={() => {
                                navigate(
                                  `/admin/interview-round-list/edit/${encrypt(element?._id)}`
                                );
                              }}
        ...
        <CustomPagination
          totalCount={totaInterviewRoundCount}
          pageSize={limit}
          currentPage={currentPage}
