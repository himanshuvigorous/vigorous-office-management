...
import { convertIntoAmount, domainName, inputAntdSelectClassNameFilter, inputClassName, inputClassNameSearch, inputLabelClassName, sortByPropertyAlphabetically } from "../../../../constents/global";
import { Controller, useForm, useWatch } from "react-hook-form";
import { branchSearch } from "../../../branch/branchManagement/branchFeatures/_branch_reducers";
import { companySearch } from "../../../company/companyManagement/companyFeatures/_company_reducers";

import { Select, Tooltip } from "antd";
import { deletepenaltyType, getpenaltyList } from "./penaltyFeatures/_penalty_reducers";
...
function PenaltyList() {
  ...
  const { penaltyListData, totapenaltyCount, loading } = useSelector((state) => state.penalty);
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deletepenaltyType(reqData)).then((data) => {
          // getpenalty()
        ...
        <div className="">
          ...
          <div className="md:flex justify-between items-center md:space-y-0 space-y-2 py-1">
            ...
            <div className="flex justify-end items-center gap-2 ">
              ...
              {canCreate &&
                <Tooltip placement="topLeft"  title='Add Penalty'>
                  <button
                    onClick={() => {
                      navigate("/admin/penalty/create");
                    }}
                    className="bg-[#074173] px-2 py-1.5 rounded-md flex justify-center items-center space-x-2 text-white"
                  >
                    <FaPlus />
                    <span className="text-[12px]">Add Penalty</span>
                  </button>
                </Tooltip>}
            </div>
          </div>
          ...
          {canRead && <table className="w-full max-w-full rounded-xl overflow-x-auto ">
            <thead className="">
              <tr className="border-b-[1px] border-[#DDDDDD] bg-[#074173] capitalize text-[12px] text-[#ffff] font-[500] h-[40px]">
                ...
                <th className="p-2 whitespace-nowrap">Penalty Name</th>
                <th className="p-2 whitespace-nowrap">Amount</th>
                <th className="p-2 whitespace-nowrap">created At</th>
                <th className="p-2 whitespace-nowrap">created By</th>
                <th className="p-2 whitespace-nowrap">status</th>
                {(canDelete || canUpdate) && <th className="p-2 whitespace-nowrap w-[10%]">Action</th>}
              </tr>
            </thead>
            {loading ? <tr className="bg-white bg-opacity-5 ">
              ...
              <tbody>
                {penaltyListData && penaltyListData?.length > 0 ? (
                  penaltyListData?.map((element, index) => (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap p-2">{element?.name || "N/A"}</td>
                      <td className="whitespace-nowrap p-2">{convertIntoAmount(element?.amount) || "N/A"}</td>
                      <td className="whitespace-nowrap p-2">{dayjs(element?.createdAt).format('DD-MM-YYYY hh:mm a')}</td>
                      <td className="whitespace-nowrap p-2">{element?.createdBy || "N/A"}</td>
                        ...
                        <span className="py-1.5 flex justify-start items-center space-x-2.5">
                          {canUpdate && <Tooltip placement="topLeft"  title='Edit'>
                            <button
                              onClick={() => {
                                navigate(
                                  `/admin/penalty/edit/${encrypt(element?._id)}`
                                );
                              }}
