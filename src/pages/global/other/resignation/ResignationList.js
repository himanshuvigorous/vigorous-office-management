...
import { domainName, inputAntdSelectClassNameFilter, showSwal, } from "../../../../constents/global";
import { Controller, useForm, useWatch } from "react-hook-form";
import { branchSearch } from "../../../branch/branchManagement/branchFeatures/_branch_reducers";
import { companySearch } from "../../../company/companyManagement/companyFeatures/_company_reducers";
import { CgComment, } from "react-icons/cg";
import { Select, Tooltip, Modal, Dropdown } from "antd";
...
function ResignationList() {
  ...
  const filters = [branchId, companyId, searchText, status].join("-");
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteResignFunc(reqData)).then((data) => {
          ...
          if (currentPage > 1 && resignationData?.length == 1) {
  ...
  return (
    <GlobalLayout onChange={onChange}>
      ...
      <div className="">
        <div className="md:flex justify-between items-center md:space-y-0 space-y-1.5 py-1">
          <div className="grid sm:grid-cols-2 grid-cols-1 sm:space-x-2 sm:space-y-0 space-y-1">
            {userType === "admin" &&
          ...
          <table className="w-full max-w-full rounded-xl overflow-x-auto">
            <thead>
              <tr className="border-b-[1px] border-[#DDDDDD] bg-header capitalize text-[#ffff] font-[500] h-[40px]">
                ...
                <th className="tableHead">Approval Date</th>
                <th className="tableHead">Notice Period(days)</th>
                <th className="tableHead">Completed Date</th>
              ...
              <tbody>
                {resignationData && resignationData?.length > 0 ? (
                  resignationData?.map((element, index) => (
                    <React.Fragment key={element?._id}>
                      ...
                      >
                        ...
                        <td className="tableData">{element?.approvalDate ? moment(element?.approvalDate).format("DD-MM-YYYY") : "-"}</td>
                        <td className="tableData">{element?.noticePeriod}</td>
                        <td className="tableData">{element?.completeDate ? moment(element?.completeDate).format("DD-MM-YYYY") : "-"}</td>

                        <td className="tableData">{moment(element?.createdAt).format("DD-MM-YYYY hh:mm a")}</td>
                        <td className="tableData">{element?.createdBy}</td>
                        <td className="tableData  border-none">
                          <span
                  ...
                  <tr className="bg-white bg-opacity-5">
                    <td colSpan={10} className="px-6 py-2 whitespace-nowrap text-center font-[600] text-sm text-gray-500">
                      Record Not Found
                    </td>
                  </tr>
                )}
