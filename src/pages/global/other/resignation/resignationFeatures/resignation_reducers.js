...
export const getEmployeeResignationList = createAsyncThunk(
  "user/getEmployeeResignationList",
  async (reqData, { rejectWithValue }) => {
    try {
      const respose = await resignServices.getEmployeeResignationList(reqData);
      return respose;
    } catch (error) {
      // showNotification({
      //   message: error?.data?.message,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getResignationList.rejected, (state, action) => {
       ...
       .addCase(getEmployeeResignationList.pending, (state, action) => {
        state.loading = true;
        state.employeeResignationData = [];
        state.employeeLoading = 0;
      })
      .addCase(getEmployeeResignationList.fulfilled, (state, action) => {
              state.employeeResignationData = action.payload?.data?.docs;
              state.totalemployeeResignCount = action.payload?.data?.totalDocs;
              state.employeeLoading = false;
      })
      .addCase(getEmployeeResignationList.rejected, (state, action) => {
        state.employeeLoading = false;
        state.employeeResignationData = [];
        state.totalemployeeResignCount = 0;
      })
