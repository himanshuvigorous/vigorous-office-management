...
import { deleteprojectCategoryFunc, getprojectCategoryListFunc } from "./projectCategoryFeatures/_projectCategory_reducers";
...
function ProjectCategory() {
  ...
  const { projectCategoryList, totalprojectCategoryCount, loading } = useSelector((state) => state.projectCategory);
  ...
  const fetchprojectCategory = () => {
    ...
    dispatch(getprojectCategoryListFunc(reqData));
  };
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteprojectCategoryFunc(reqData)).then((data) => {
          // fetchprojectCategory();

          if (currentPage > 1 && projectCategoryList?.length == 1) {
  ...
  return (
    <GlobalLayout onChange={onChange}>
      ...
      <div className="bg-[#ffffff] w-full overflow-x-auto mt-1 rounded-xl">
        {canRead && <table className="w-full max-w-full rounded-xl overflow-x-auto ">
          ...
          {loading ? <tr className="bg-white bg-opacity-5 ">
            ...
            <tbody>
              {projectCategoryList && projectCategoryList?.length > 0 ? (
                    ...
                    {(canUpdate || canDelete) && <td className="whitespace-nowrap border-none p-2">
                      <span className="py-1.5 flex justify-start items-center space-x-2">
                        {canUpdate && <button
                          onClick={() => {
                            navigate(
                              `/admin/projects-category/edit/${encrypt(element?._id)}`
                            );
                          }}
      ...
      {projectCategoryList?.length > 0 &&
        <CustomPagination
          totalCount={totalprojectCategoryCount}
          pageSize={limit}
