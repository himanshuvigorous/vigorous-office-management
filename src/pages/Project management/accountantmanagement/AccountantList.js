...
import { deleteAccountantFunc, getAccountantListFunc } from "./accountManagentFeatures/_accountManagement_reducers";
...
function AccountantList() {
  ...
  const { accountantList, totalAccountantCount, loading } = useSelector((state) => state.accountManagement);
  ...
  const fetchprojectCategory = () => {
    ...
    dispatch(getAccountantListFunc(reqData));
  };
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteAccountantFunc(reqData)).then((data) => {
          // fetchprojectCategory();

          if (currentPage > 1 && accountantList?.length == 1) {
  ...
  return (
    <GlobalLayout onChange={onChange}>
          ...
          {loading ? <tr className="bg-white bg-opacity-5 ">
            ...
            <tbody>
              {accountantList && accountantList?.length > 0 ? (
                accountantList?.map((element, index) => (
                  <tr
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap border-none p-2 ">
                      {element?.accountentData?.fullName ?? "-"}
                    </td>
                    <td className="whitespace-nowrap border-none p-2 ">
                      {element?.accountentData?.email ?? "-"}
                    </td>
                    <td className="whitespace-nowrap border-none p-2 ">
                    ...
                    {(canUpdate || canDelete) && <td className="whitespace-nowrap border-none p-2">
                      <span className="py-1.5 flex justify-start items-center space-x-2">
                        {canUpdate && <button
                          onClick={() => {
                            navigate(
                              `/admin/accountant-management/edit/${encrypt(element?._id)}`
                            );
                          }}
                        ...
                        {canUpdate && <button
                          onClick={() => {
                            navigate(
                              `/admin/accountant-management/report/${encrypt(JSON.stringify(element))}`
                            );
                          }}
                          className="px-2 py-1.5 text-xs rounded-md bg-transparent border border-muted"
                          type="button"
      ...
      {accountantList?.length > 0 &&
        <CustomPagination
          totalCount={totalAccountantCount}
          pageSize={limit}
