...
import { deleteprojectserviceFunc, getprojectserviceListFunc } from "./projectserviceFeatures/_projectservice_reducers";
...
function Projectservice() {
  ...
  const { projectserviceList, totalprojectserviceCount, loading } = useSelector((state) => state.projectservice);
  ...
  const fetchprojectservice = () => {
    ...
    dispatch(getprojectserviceListFunc(reqData));
  };
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteprojectserviceFunc(reqData)).then((data) => {
          // fetchprojectservice();

          if (currentPage > 1 && projectserviceList?.length == 1) {
  ...
  return (
    <GlobalLayout onChange={onChange}>
          ...
          {loading ? <tr className="bg-white bg-opacity-5 ">
            ...
            <tbody>
              {projectserviceList && projectserviceList?.length > 0 ? (
                projectserviceList?.map((element, index) => (
                  <tr
                    ...
                    {(canUpdate || canDelete) && <td className="whitespace-nowrap border-none p-2">
                      <span className="py-1.5 flex justify-start items-center space-x-2">
                        {canUpdate && <button
                          onClick={() => {
                            navigate(
                              `/admin/projects-services/edit/${encrypt(element?._id)}`
                            );
                          }}
      ...
      {projectserviceList?.length > 0 &&
        <CustomPagination
          totalCount={totalprojectserviceCount}
          pageSize={limit}
