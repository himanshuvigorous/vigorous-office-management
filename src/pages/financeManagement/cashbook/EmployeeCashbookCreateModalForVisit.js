...
const EmployeeCashbookCreateModalForVisit = ({ visible, onCancel, onSuccess }) => {
 const {
    register,
    handleSubmit,
    control,
  ...
  const onSubmit = (data) => {
    const finalPayload = {
      companyId: visible?.data?.companyId,
      directorId: visible?.data?.directorId,
      branchId: visible?.data?.branchId,
      "employeId":( Array.isArray(visible?.data?.employeIds) && visible?.data.employeIds.length > 0) ? visible?.data.employeIds[0]  :null ,
      ...
      date: data?.date,
      status: "Pending",
      attachment: fileList.map(file => file.uid),
      visitorId:visible?.data?._id
    };
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const res = await dispatch(fileUploadFunc(reqData));
      if (res?.payload?.data) {
        setFileList(prev => [...prev, {
          uid: res.payload.data,
          name: file.name,
          status: 'done',
          url: `${process.env.REACT_APP_BACKEND_DOMAIN_NAME}/public/${res.payload.data}`
        }]);
      }
 ...
 const handleRemoveFile = (file) => {
  ...
  return (
    <Modal
      title="Create Employee Cashbook"
            ...
            <div className="w-full">
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  menuPosition="fixed"             // Prevent relative positioning
                  styles={{
              ...
              {fileList?.map(file => (
                <Card 
                  ...
                  actions={[
                    ...
                    <DeleteOutlined onClick={() => handleRemoveFile(file)} />
                  ]}
                >
...
export default EmployeeCashbookCreateModalForVisit;